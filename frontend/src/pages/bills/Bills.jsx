...
import { BillsGhost } from "../../components/ui/GhostLoader.jsx";
...
function BillSplitsList({ billId, splitStatusMutation }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-100 flex flex-col gap-2">
      <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide transition-colors">
        Split Details
      </p>
      {splits.map((s) => (
        <div
          key={s.id}
          className="flex items-center justify-between text-sm bg-gray-50 dark:bg-slate-900/40 p-2.5 rounded-lg border border-gray-100 dark:border-slate-700 transition-colors"
        >
...
export default function Bills() {
  ...
  const [selectedProperty, setSelectedProperty] = useState(
    () => sessionStorage.getItem("bills_property") || "",
  );
  const [selectedUnit, setSelectedUnit] = useState(
    () => sessionStorage.getItem("bills_unit") || "",
  );
  ...
  const handleEditSplitTypeChange = (split_type) => {
    ...
    if (split_type === "custom") {
      setEditCustomSplits(
        unitTenants.map((t) => ({ tenant_id: t.id, name: t.name, amount: "" })),
      );
    } else {
      setEditCustomSplits([]);
    }
  ...
  const handleUpdateBill = async (e) => {
    ...
    const payload = {
      ...
      split_type: editBillForm.split_type,
      month: Number(editBillForm.month),
      year: Number(editBillForm.year),
      note: editBillForm.note || undefined,
    };
    if (editBillForm.split_type === "custom") {
      payload.splits = editCustomSplits.map((s) => ({
        tenant_id: s.tenant_id,
    ...
    <div className="max-w-4xl mx-auto flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 transition-colors">
          View all bills across your properties
        </p>
      </div>
      ...
      {isFetching && bills.length === 0 && <BillsGhost />}
      ...
      {bills.length > 0 && (
        <div
          className={`flex flex-col gap-8 transition-opacity duration-200 ${isFetching ? "opacity-50 pointer-events-none" : "opacity-100"}`}
        >
          {Object.entries(groupedBills).map(([propertyName, units]) => (
            <div
              key={propertyName}
              className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden transition-colors"
            >
              ...
              <div className="p-5 flex flex-col gap-6">
                {Object.entries(units).map(([unitName, unitBills]) => (
                  <div key={unitName}>
                    <h3 className="text-md font-semibold text-gray-700 dark:text-gray-300 mb-3 border-b border-gray-100 dark:border-slate-700 pb-1 transition-colors">
                      {unitName}
                    </h3>
                    <div className="flex flex-col gap-3">
                      {unitBills.map((bill) => (
                        <div
                          key={bill.id}
                          className="border border-gray-100 dark:border-slate-700 rounded-lg p-4 hover:border-gray-300 dark:hover:border-slate-500 transition-colors bg-white dark:bg-slate-800/40"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            {/* Bill Info & Names */}
                            <div className="flex flex-col gap-1.5">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-semibold text-gray-900 dark:text-white capitalize transition-colors">
                                  {bill.type}
                                </span>
                                <Badge variant={TYPE_COLORS[bill.type]}>
                                  {bill.type}
                                </Badge>
                            ...
                            {/* Actions & Amounts */}
                            <div className="flex items-center gap-3">
                              <span className="text-base font-bold text-gray-900 dark:text-white mr-2 transition-colors">
                              ...
                              <button
                                ...
                                className="text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 p-1.5 cursor-pointer bg-gray-50 dark:bg-slate-700 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded-md transition-colors"
                              >
                                <FiEdit2 size={16} />
                              </button>
                              ...
                              <button
                                ...
                                className="text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 p-1.5 cursor-pointer bg-gray-50 dark:bg-slate-700 hover:bg-red-50 dark:hover:bg-red-900/40 rounded-md transition-colors"
                              >
                                <FiTrash2 size={16} />
                              </button>
                            </div>
                          ...
                          {bill.split_type !== "unit" && (
                            <BillSplitsList
                              billId={bill.id}
                              splitStatusMutation={splitStatusMutation}
                            />
                          )}
                        </div>
                      ))}
      ...
      <Modal
        open={editBillModal}
        onClose={() => setEditBillModal(false)}
        title="Edit Bill"
      >
        <form onSubmit={handleUpdateBill} className="space-y-4">
          <div>
            ...
            <select
              ...
              value={editBillForm.type}
              onChange={(e) =>
                setEditBillForm({ ...editBillForm, type: e.target.value })
              }
            >
              {BILL_TYPES.map((t) => (
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setEditBillForm({ ...editBillForm, amount: e.target.value })
              }
          ...
          <div>
            ...
            <select
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white cursor-pointer"
              value={editBillForm.split_type}
              onChange={(e) => handleEditSplitTypeChange(e.target.value)}
            >
          ...
          {editBillForm.split_type === "custom" &&
            editCustomSplits.length > 0 && (
              <div className="flex flex-col gap-2 bg-slate-50 p-3 rounded-lg border border-slate-100">
                ...
                <div className="mt-1 pt-2 border-t border-slate-200 flex justify-between items-center">
                  ...
                  <span
                    className={`text-sm font-bold ${editCustomSplits.reduce((s, c) => s + (Number(c.amount) || 0), 0) === Number(editBillForm.amount) ? "text-green-600" : "text-red-600"}`}
                  >
                    {formatCurrency(
                      editCustomSplits.reduce(
                        (s, c) => s + (Number(c.amount) || 0),
                        0,
                      ),
                    )}{" "}
                    / {formatCurrency(Number(editBillForm.amount) || 0)}
                  </span>
                </div>
              </div>
            )}
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setEditBillForm({ ...editBillForm, note: e.target.value })
              }
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              placeholder="e.g., Q3 Water Tax"
            />
          </div>
